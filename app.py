...
try:
    with open("assets/logo.jpeg", "rb") as image_file:
...
with col_left:
    ...
    has_diff = "diff_context"         in values
    has_sec  = "security_report"      in values
    has_crag = "crag_enhanced_context" in values

    st.markdown(
        f"<div class='pipe-step {'pipe-active' if has_diff and not has_sec else ''}'>"
        f"1. Fetcher Node {'✅' if has_diff else '⏳'}</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<div class='pipe-step {'pipe-active' if has_sec and not has_crag else ''}'>"
        f"2. Security & Style Agents {'✅' if has_sec else '⏳'}</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"<div class='pipe-step {'pipe-active' if has_crag and not is_waiting and not is_done else ''}'>"
        f"3. CRAG Evaluator {'✅' if has_crag else '⏳'}</div>",
        unsafe_allow_html=True,
    )

    if is_waiting:
        st.markdown(
            "<div class='pipe-step pipe-active' style='border-color:#f59e0b;'>"
            "4. HITL Breakpoint (BLOCKED) ⚠️</div>",
            unsafe_allow_html=True,
        )
    elif is_done:
        st.markdown(
            "<div class='pipe-step' style='border-color:#10b981;'>"
            "4. Completed ✅</div>",
            unsafe_allow_html=True,
        )
...
with col_right:
        ...
        with st.form("hitl_form"):
            ...
            if st.form_submit_button("Transmit Decision", type="primary"):
                if not reviewer:
                    ...
                    is_approved = decision == "Approve Merge"
                    try:
    ...
    if not values:
        ...
        if "security_report" in values:
            with st.container(border=True):
                st.markdown("**🛡️ Security Agent**")
                st.markdown(values["security_report"])

        if "performance_report" in values:
            with st.container(border=True):
                st.markdown("**⚡ Performance Agent**")
                st.markdown(values["performance_report"])

        if "style_report" in values:
            with st.container(border=True):
                st.markdown("**🎨 Style & Quality Agent**")
                st.markdown(values["style_report"])

        if "crag_enhanced_context" in values:
            with st.container(border=True):
                st.markdown("**🔍 CRAG Grounding Evaluator**")
                st.markdown(values["crag_enhanced_context"])

        if "final_report_markdown" in values:
            with st.container(border=True):
                st.markdown("**📋 Final Aggregated Report**")
                st.markdown(values["final_report_markdown"])
